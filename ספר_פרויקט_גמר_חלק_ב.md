...
│  8. Deviation Analysis → Identify unexpected peaks         │
│  9. Visualize → 4-Tab MATLAB Interface                    │
│ 10. Save → CSV + Plots + Log                              │
└───────────────────────────────────────────────────────────┘
```

#### מבנה קוד עיקרי

**ESP32 (esp32_usb_serial_safe_V2.ino):**
```cpp
void setup() {
  Serial.begin(115200);
  Wire.begin();
...
#### קוד עיקרי - esp32_usb_serial_safe_V2.ino
...
def classify_tremor(self, metrics):
    ...
    if ratio > 2.0:
        ...
        return {
            'type': 'Inconclusive - Requires Clinical Correlation',
            'confidence': 'Low',
            'ratio': ratio
        }
```
...
- זה הסטנדרט במחקר הרפואי
- אזור החפיפה (6-7 Hz) מטופל באמצעות Power Ratio
- Deviation Analysis מזהה פיקים חריגים באזור זה בזמן אימות
- שימור הטווחים הקליניים מאפשר השוואה למחקרים קיימים
...
## 5.2 אימות תכן לעומת אימות קליני - הבהרת מטרות הפרויקט
...
- חישוב Sensitivity, Specificity, PPV, NPV
- דורש אישור IRB ושיתוף פעולה עם מוסד רפואי

### סיכום:

פרויקט זה מוכיח ש**הטכנולוגיה עובדת** ומספק **אב-טיפוס תפקודי** (Functional Prototype) למערכת זיהוי רעד. זהו יסוד הכרחי לפני מעבר לשלב האימות הקליני.

**המסר המרכזי:** אימות תכן הוא שלב קריטי ומספיק לפרויקט גמר הנדסי. אימות קליני הוא שלב נפרד שדורש משאבים רפואיים ומחקריים נוספים.

---

## 5.2.5 Deviation Analysis - מתודולוגיית בדיקת סטיות

בנוסף לאימות הכללי של הסיווג (Rest vs Essential), פיתחנו שיטה מפורטת לזיהוי ותיעוד פיקים חריגים ובדיקת מיקומם ביחס לאזור החפיפה הבעייתי.

### מטרת השיטה:

**מודעות לפערים** - השיטה לא מטרתה להשיג דיוק מוחלט, אלא להיות מודעים לסטיות ולפיקים מחוץ לתחום הצפוי, במיוחד כאשר הם נמצאים באזור החפיפה הבעייתי (6-7 Hz).

### האלגוריתם:

**שלב 1: הגדרת תחום צפוי (Expected Band)**
```
Rest Tremor sequence    → Expected: [4.0, 6.0] Hz
Essential sequence      → Expected: [8.0, 10.0] Hz
```

**שלב 2: זיהוי פיקים ב-PSD**
המערכת מזהה את כל הפיקים המשמעותיים (peaks) בצפיפות הספק הספקטרלית (PSD).

**שלב 3: סיווג פיקים**
```python
for each peak in detected_peaks:
    if peak in expected_band:
        status = "✓ בתוך התחום הצפוי"
    else:
        status = "⚠️ פיק זר (מחוץ לתחום)"
        # חישוב סטייה מקצה התחום הקרוב ביותר
        deviation = min(abs(peak - band_low), abs(peak - band_high))

        # בדיקה בינארית: האם הפיק באזור החפיפה?
        in_overlap_zone = (6.0 <= peak <= 7.0)

        if in_overlap_zone:
            flag = "🚩 דגל אדום - פיק באזור חפיפה!"
```

### דוגמאות מעשיות:

**דוגמה 1: Rest Sequence (4-6 Hz)**
```
פיקים שזוהו: [4.25, 5.00, 5.75] Hz
...
מסקנה: כל הפיקים בתחום הצפוי - תוצאה נקייה ✅
```

**דוגמה 2: Rest Sequence עם פיק באזור חפיפה**
```
פיקים שזוהו: [4.25, 6.00, 6.75] Hz

ניתוח:
• 4.25 Hz → בתוך [4,6] ✓ OK
• 6.00 Hz → בתוך [4,6] ✓ OK
• 6.75 Hz → מחוץ ל-[4,6] ⚠️
  ├─ Deviation: 6.75 - 6.0 = 0.75 Hz
  └─ ב-[6,7]? כן! 🚩 דגל אדום

מסקנה: יש פיק זר באזור החפיפה - דורש תשומת לב ⚠️
```

**דוגמה 3: Essential Sequence עם פיק בתדר נמוך**
```
פיקים שזוהו: [6.5, 8.50, 9.25] Hz

ניתוח:
• 8.50 Hz → בתוך [8,10] ✓ OK
• 9.25 Hz → בתוך [8,10] ✓ OK
• 6.5 Hz → מחוץ ל-[8,10] ⚠️
  ├─ Deviation: 8.0 - 6.5 = 1.5 Hz
  └─ ב-[6,7]? כן! 🚩 דגל אדום

מסקנה: פיק זר ב-6.5 Hz - עשוי להצביע על רכיב Rest ⚠️
```

### יישום בטבלאות התוצאות:

בטבלאות 6 ו-7 להלן, מוצגים התוצאות הבאות לכל סגמנט:
- **Peaks in Band:** פיקים שנמצאו בתוך התחום הצפוי (✓)
- **Unexpected Peaks:** פיקים מחוץ לתחום (תדר בדויק)
- **In Overlap (6-7 Hz)?:** תשובה בינארית - כן/לא

שיטה זו מאפשרת לנו לזהות מקרים בעייתיים שעשויים לדרוש בדיקה נוספת או אימות קליני מעמיק יותר.

---

## 5.3 אימות עם מנוע - חיישן על אצבע

### 5.3.1 סדרת רעד מנוחה (4-6 Hz)
...
| Segment | Motor Freq | Detected Freq | Error | Peaks in Band [4-6] | Unexpected Peaks | In Overlap (6-7)? | Classification |
|---------|------------|---------------|-------|---------------------|------------------|-------------------|----------------|
| 1 | 4.0 Hz | 4.25 Hz | +0.25 Hz | 4.25 Hz ✓ | - | No | Rest ✅ |
| 2 | 5.0 Hz | 5.25 Hz | +0.25 Hz | 5.25 Hz ✓ | - | No | Rest ✅ |
| 3 | 6.0 Hz | 6.00 Hz | 0.00 Hz | 6.00 Hz ✓ | 6.75 Hz | **Yes** ⚠️ | Rest ⚠️ |
| 4 | 5.0 Hz | 5.00 Hz | 0.00 Hz | 5.00 Hz ✓ | - | No | Rest ✅ |

**הערות:**
- **Expected Band:** 4.0-6.0 Hz (Rest Tremor range)
- **Segment 3 - Deviation Analysis:**
  - פיק צפוי: 6.00 Hz (בתוך התחום ✓)
  - **פיק זר:** 6.75 Hz (מחוץ לתחום [4-6])
  - Deviation: 6.75 - 6.0 = **0.75 Hz** מעבר לקצה העליון
  - **דגל אדום:** הפיק נמצא באזור החפיפה הבעייתי (6-7 Hz)
  - סיבה אפשרית: דמפינג ביומכני יוצר הרמוניות מנוע ב-6.75 Hz
...
### 5.3.2 סדרת רעד אסנציאלי (8-10 Hz)
...
| Segment | Motor Freq | Detected Freq | Error | Peaks in Band [8-10] | Unexpected Peaks | In Overlap (6-7)? | Classification |
|---------|------------|---------------|-------|----------------------|------------------|-------------------|----------------|
| 1 | 8.0 Hz | 8.00 Hz | 0.00 Hz | 8.00 Hz ✓ | - | No | Essential ✅ |
| 2 | 9.0 Hz | 8.75 Hz | -0.25 Hz | 8.75 Hz ✓ | - | No | Essential ✅ |
| 3 | 10.0 Hz | 10.00 Hz | 0.00 Hz | 10.00 Hz ✓ | - | No | Essential ✅ |
| 4 | 9.0 Hz | 9.00 Hz | 0.00 Hz | 9.00 Hz ✓ | - | No | Essential ✅ |

**הערות:**
- **Expected Band:** 8.0-10.0 Hz (Essential Tremor range)
- **Deviation Analysis:** לא זוהו פיקים זרים מחוץ לתחום הצפוי
- **אזור חפיפה:** אין פיקים באזור החפיפה הבעייתי (6-7 Hz)
- **מסקנה:** התוצאות נקיות ללא חריגות - סיווג Essential Tremor ברור ✅
...
**קבצים עיקריים:**
- `esp32_usb_serial_safe_V2.ino` - Firmware לESP32
...
**הערה:** דוגמאות אלו ממחישות את יכולת המערכת לזהות ולסווג בבירור את שני סוגי הרעד המובחנים (Rest vs Essential). מקרים עם Power Ratio בטווח הביניים (0.5-2.0) ידרשו בדיקה קלינית נוספת.
